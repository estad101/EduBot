...
async def get_support_notifications(db: Session = Depends(get_db)):
    """
    Get support ticket notifications for dashboard alerts.
    """
    try:
...
async def get_support_tickets(
    ...
    """
    try:
        tickets, total = SupportService.get_all_tickets(db, skip=skip, limit=limit, status=status)
...
async def add_support_message(
    ...
    """
    Add an admin message to a support ticket and send WhatsApp notification.
    """
    try:
        ...
        support_msg = SupportService.add_message(
            db,
            ticket_id=ticket_id,
            sender_type="admin",
            message=message,
        ...
        return StandardResponse(
            status="success",
            message="Message added and notification sent",
            data={"message_id": support_msg.id}
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def close_support_ticket(
    ...
    """
    Close a support ticket and mark as resolved.
    """
    try:
        ticket = SupportService.get_ticket(db, ticket_id)
        if not ticket:
        ...
        updated_ticket = SupportService.update_ticket_status(
            db,
            ticket_id,
        ...
        try:
            ...
            await WhatsAppService.send_message(
                ...
                message=(
                    ...
                    f"If you have any other questions, feel free to reach out again.\n\n"
                    f"Thank you for using EduBot! 🙏"
                )
            )
        except Exception as e:
            logger.warning(f"⚠️ Could not send closing message: {str(e)}")
        
        return StandardResponse(
            status="success",
            message="Ticket closed and marked as resolved",
            data={"ticket_id": updated_ticket.id, "status": updated_ticket.status}
        )
    except HTTPException:
        raise
    except Exception as e:
